...
export class ModuleInstallComponent {
    ...
    install() {
        ...
        this.moduleService.installModule(this.module).subscribe((res) => {
                this.loaderService.close();
                this.notification.showInfo('Module successfully installed');
                this.router.navigate(['..', res.body.id, 'view']);
            },
            (err: any) => {
                this.loaderService.close();
                this.notification.showError('Could not install module');
            }
        );
    }
